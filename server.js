...
app.listen(port, '0.0.0.0', async () => {
  ...
  logger.info(env.useTestnet ? 'Binance TESTNET (demo) modu AKTIF.' : 'Binance LIVE modu AKTIF.');
  logger.info(`Trading=${env.tradingEnabled} | DryRun=${env.dryRun} | SheetsRequired=${env.sheetRequired}`);
  ...
  if (telegramService.isConfigured()) {
    telegramService
      .sendTelegramMessage(telegramService.formatStartupNotification())
      .catch((err) => logger.warn('[TELEGRAM] startup bildirimi hatasi: ' + err.message));
  }
});
